...
from deeplift.blobs.convolution import PaddingMode, PoolMode
...
class TestDense(unittest.TestCase):
    ...
    def test_relu_after_conv1d(self): 
        ...
        conv_layer = blobs.Conv1D(
                        W=np.random.random((2,2,2)).astype("float32"),
    ...
    def test_relu_after_conv2d(self): 
        input_layer = blobs.Input(num_dims=None,
                                  shape=(None,2,2,2))
        conv_layer = blobs.Conv2D(
                        W=np.random.random((2,2,2,2)).astype("float32"),
                        b=np.random.random((2,)).astype("float32"),
                        conv_mxts_mode=ConvMxtsMode.Linear,
                        strides=(1,1),
                        padding_mode=PaddingMode.valid)
