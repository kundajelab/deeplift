...
\begin{aligned}
A_{t_i} &= \frac{e^{A_{y_i}}}{\sum_{i' = 1}^n e^{A_{y_i'}}}\\
           &= \frac{e^{w_{xy_i}A_x + r_{y_i}}}{\sum_{i' = 1}^n e^{w_{xy_{i'}}A_x + r_{y_{i'}}}}\\
           &= \frac{e^{w_{xy_i}A_x + r_{y_i}}}{\sum_{i' = 1}^n e^{w_{xy_i}A_x + r_{y_{i'}}}}\\
           &= \frac{e^{r_{y_i}}}{\sum_{i' = 1}^n e^{r_{y_{i'}}}} 
\end{aligned}
\end{equation}
...
A model with the VGG16 [cite] architecture was trained on a scaled-down version of the Imagenet dataset, dubbed `Tiny Imagenet'. The images were $64 \times 64$ in dimension and belonged to one of 200 output classes. Pictured are importance scores computed using three different approaches.
...
\begin{figure}[!htmb]
\begin{center}
\includegraphics[scale=0.3]{LIFTPADSequence2.png}
\includegraphics[scale=0.3]{GradientSequence2.png}
\caption{LIFTPAD scores (top) and gradient-based scores (bottom) are plotted for each position in the DNA sequence and colored by the DNA base.}
\end{center}
\end{figure}
 
\subsection{Extension to Recurrent Neural Networks}
