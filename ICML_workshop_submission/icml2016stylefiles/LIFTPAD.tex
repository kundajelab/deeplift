...
\caption{Simple network with inputs $x_1$ and $x_2$ that have reference values of 0. When $x_1 = x_2 = -1$, the output is 0.1 but the gradients w.r.t $x_1$ and $x_2$ are 0 due to the inactive ReLU $y$ (which has an activation of $2$ under the reference input). By comparing activations to their reference values, LIFTPAD assigns contributions of $\left(0.1\frac{1}{3}\right)$ and $\left(0.1\frac{2}{3}\right)$ to $x_1$ and $x_2$ respectively.}
\end{center}
\end{figure}
